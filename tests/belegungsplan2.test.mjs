...
test('Belegungsplan 2 contains the conflict-adjusted proposal', () => {
  ...
  assert.doesNotMatch(proposalSource, /'fussball\/herren\/kreisliga-2\|Donnerstag':/);
  assert.doesNotMatch(proposalSource, /'fussball\/herren\/bezirksliga\|Dienstag':/);
  assert.doesNotMatch(proposalSource, /'fussball\/herren\/bezirksliga\|Donnerstag':/);
  ...
  assert.match(proposalSource, /Eine Überbelegung bleibt sichtbar/);
  assert.match(proposalSource, /zusammen 150 % des Hauptplatzes/);
});
