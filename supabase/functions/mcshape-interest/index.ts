...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const mcShapeRecipient = Deno.env.get('MCSHAPE_RECIPIENT_EMAIL');
    const bsvConfirmationRecipient = Deno.env.get('BSV_CONFIRMATION_EMAIL');
    ...
    const interestLabel = escapeHtml(interest || 'Noch offen');
    const contactTimeLabel = escapeHtml(preferredContactTime || 'Flexibel');
